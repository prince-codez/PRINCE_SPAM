...
def load_plugins(plugin_name):
    path = Path(f"PRINCE/modules/{plugin_name}.py")
    spec = importlib.util.spec_from_file_location(f"PRINCE.modules.{plugin_name}", path)
    load = importlib.util.module_from_spec(spec)
    load.logger = logging.getLogger(plugin_name)
    spec.loader.exec_module(load)
    sys.modules["PRINCE.modules." + plugin_name] = load
    print("Altron has Imported " + plugin_name)


files = glob.glob("PRINCE/modules/*.py")
for name in files:
    with open(name) as a:
        patt = Path(a.name)
...
print("\n𝐏𝐫𝐢𝐧𝐜𝐞 𝐒𝐩𝐚𝐦 𝐁𝐨𝐭𝐬 𝐃𝐞𝐩𝐥𝐨𝐲𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲 ⚡\nMy Master ---> @PRINCE_WEBZ")
